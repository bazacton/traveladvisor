 ...
 * </code>
 * Example : Note : define your Prefix (like tabs || tab_item ) in pattren. (ref. traveladvisor_get_pattern())
 ...
 *  [/tabs]';
 * 	$Output = array();
 *  $PREFIX = 'prefix'; //user prefix as traveladvisor_message OR traveladvisor_tabs | tab_item
 * 	$parseInstance 	= new ShortcodeParse();
 * 	$output = $b->traveladvisor_shortcodes( $output, $text );
 * 	echo '<pre>';
...
if ( !class_exists( 'ShortcodeParse' ) ) {
	...
	class ShortcodeParse {
		...
		function traveladvisor_get_pattern( $content, $PREFIX ) {
		...
		function traveladvisor_parse_atts( $content ) {
		...
		function traveladvisor_shortcodes( &$output, $content, $child = true, $PREFIX ) {
			$patts = $this->traveladvisor_get_pattern( $content, $PREFIX );
			$t = array_filter( $this->traveladvisor_get_pattern( $content, $PREFIX ) );
			if ( !empty( $t ) ) {
				...
				foreach ( $parents as $k => $parent ) {
					...
					$t = array_filter( $this->traveladvisor_get_pattern( $contents[$k], $PREFIX ) );
					$t_s = $this->traveladvisor_shortcodes( $outputNew, $contents[$k], true, $PREFIX );
					$output[$name] = array( 'name' => $parents[$k] );
					$output[$name]['atts'] = $this->traveladvisor_parse_atts( $atts[$k] );
