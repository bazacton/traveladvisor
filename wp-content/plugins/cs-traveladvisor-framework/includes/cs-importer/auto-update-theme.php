...
$traveladvisor_plugins_upgraded = array();

add_action( 'admin_init', 'traveladvisor_check_for_updates' );
if ( ! function_exists( 'traveladvisor_check_for_updates' ) ) {
	/**
	 * Check for theme updates if available
	 */
	function traveladvisor_check_for_updates() {
		global $traveladvisor_options;
		...
		if ( $upgrade_option ) {
			...
			if ( 'themes.php' == basename( $_SERVER[ PHP_SELF ] ) ) {
				add_action( 'admin_footer', 'traveladvisor_show_theme_auto_update_available_on_themes_page' );
			}

			add_action( 'admin_notices', 'traveladvisor_show_theme_auto_update_available' );
			return;
		}

		//- delete_option( 'last_time_auto_theme_update_checked_' . THEME_NAME );
		if ( ! empty( $traveladvisor_options['traveladvisor_marketplace_username'] ) ) {
			...
			$user_name = $traveladvisor_options['traveladvisor_marketplace_username'];
			$api_key  = $traveladvisor_options['traveladvisor_secret_api_key'];
			$skip_backup = $traveladvisor_options['traveladvisor_skip_theme_backup'];
			...
			if ( $errors = $protected_api->api_errors() ) {
				add_action( 'admin_notices', 'traveladvisor_show_theme_auto_update_error' );
			} else {
			...
			add_action( 'admin_notices', 'traveladvisor_show_theme_auto_update_warning' );
		}
	} // End Function.
} // End If.

if ( ! function_exists( 'traveladvisor_show_theme_auto_update_available_on_themes_page' ) ) {
	/**
	 * Show theme update available on themes page, Ouput JS to footer of themes.php page.
	 */
	function traveladvisor_show_theme_auto_update_available_on_themes_page() {
...
if ( ! function_exists( 'traveladvisor_show_theme_auto_update_available' ) ) {
	/**
	 * Show notification if an update is avaiable.
	 */
	function traveladvisor_show_theme_auto_update_available() {
...
if ( ! function_exists( 'traveladvisor_show_theme_auto_update_error' ) ) {
	/**
	 * Show error if marketplace username or secret API keys is not valid.
	 */
	function traveladvisor_show_theme_auto_update_error() {
		$class = 'notice notice-error is-dismissible';
		$message = __( 'Please provide valid Marketplace Username and Secret API Key in theme options to get Automatic Theme Updates.  &raquo', 'traveladvisor' );
//              echo '<a href="' . network_admin_url( 'admin.php?page=' . THEME_OPTIONS_PAGE_SLUG ) . '#tab-auto-updater-show">Fix Now</a>';

		printf( '<div class="%1$s"><p>%2$s</p></div>', $class, $message );
	}
}

if ( ! function_exists( 'traveladvisor_auto_upgrade_theme' ) ) {
	/**
	 * Show Warning if auto updates are not configured.
	 */
	function traveladvisor_show_theme_auto_update_warning() {
		if ( false === ( $admin_dismiss_notice = get_transient( 'admin_dismiss_notice' ) ) ) {
			
			$class = 'notice notice-warning is-dismissible';
			$message = __( 'Please configure Auto Updates in theme options to get Automatic Theme Updates. &raquo', 'traveladvisor');
...
if ( ! function_exists( 'traveladvisor_auto_upgrade_theme_and_plugins' ) ) {
	/**
	 ...
	 */
	function traveladvisor_auto_upgrade_theme_and_plugins() {
		global $traveladvisor_options, $current_screen, $traveladvisor_theme_upgraded_name, $traveladvisor_plugins_upgraded;
		...
		// Upgrade Theme.
		$theme_info['user_name'] = $traveladvisor_options['traveladvisor_marketplace_username'];
		$theme_info['api_key'] = $traveladvisor_options['traveladvisor_secret_api_key'];
		$theme_info['skip_backup'] = $traveladvisor_options['traveladvisor_skip_theme_backup'];
		$status = true;
		$status = traveladvisor_auto_update_theme( $theme_info );

		// Upgrade Plugins.
		if ( function_exists( 'get_plugins_to_be_updated' ) ) {
			traveladvisor_auto_update_plugins( get_plugins_to_be_updated() );
		}
		...
		if ( $status ) {
			...
			return array( 'traveladvisor_theme_upgraded_name' => $traveladvisor_theme_upgraded_name, 'traveladvisor_plugins_upgraded' => $traveladvisor_plugins_upgraded );
		} else {
			return array();
		}
	} // End Function.
...
if ( ! function_exists( 'traveladvisor_auto_update_theme' ) ) {
	/**
	 * Update theme
	 *
	 ...
	 */
	function traveladvisor_auto_update_theme( $theme_info ) {
		global $traveladvisor_theme_upgraded_name;
		...
		$title = __( 'Update Theme', 'traveladvisor' );
		...
		if ( ! $skip_backup  ) {
			traveladvisor_backup_theme( $theme );
		}
		...
		$traveladvisor_theme_upgraded_name = $theme;
...
if ( ! function_exists( 'traveladvisor_auto_update_plugins' ) ) {
	/**
	 * Update plugins
	 *
	 ...
	 */
	function traveladvisor_auto_update_plugins( $plugins ) {
		global $traveladvisor_plugins_upgraded;
		...
		foreach ( $plugins as $key => $plugin_info ) {
			if ( '' != $plugin_info['slug'] ) {
				...
				array_push( $traveladvisor_plugins_upgraded, $plugin['name'] . ' Plugin' );
			}
		}
		return true;
	} // End Function.
...
if ( ! function_exists( 'traveladvisor_backup_theme' ) ) {
	/**
	 ...
	 */
	function traveladvisor_backup_theme( $theme ) {
		$backup_errors = array();

		require_once 'class-envato-backup.php';
