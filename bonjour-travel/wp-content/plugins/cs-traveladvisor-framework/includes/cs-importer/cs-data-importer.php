<?php
/**
 * Contains CS_Data_Importer class
 *
 * @since	1.2
 * @package	WordPress
 */
...
/**
 * Class CS_Data_Importer class
 *
 * @since	1.2
 */
class CS_Data_Importer {
	/**
	 * Demo data name
	 *
	 ...
	 */
	public $traveladvisor_importer_class_path = '';
	...
	function __construct() {
		...
		$this->traveladvisor_importer_class_path 	= WP_PLUGIN_DIR . '/cs-traveladvisor-framework/includes/cs-importer/wordpress-importer.php';
	}
	...
	function import() {
		...
		if ( $this->is_content ) {
			...
			if ( $this->action_return ) {
				...
				$this->set_up_pages();
				ob_end_clean();
				$this->make_output( true, __( 'WP data successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry importer class missing.', 'traveladvisor' ) );
			}
		}
		...
		if ( $this->is_widgets ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Widgets successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry widgets class missing.', 'traveladvisor' ) );
			}
		}
		...
		if ( $this->is_theme_options ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Theme options successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry theme options file not readable.', 'traveladvisor' ) );
			}
		}
		...
		if ( $this->is_users ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Users successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry users file not readable.', 'traveladvisor' ) );
			}
		}
		...
		if ( $this->is_menus ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Menus successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry menus was not imported.', 'traveladvisor' ) );
			}
		}
		...
		if ( $this->is_plugins ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Plugin Options successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry plugin options was not imported.', 'traveladvisor' ) );
			}
		}
		...
		if ( $this->is_sliders ) {
			...
			ob_end_clean();
			if ( $this->action_return ) {
				$this->make_output( true, __( 'Sliders successfully got imported.', 'traveladvisor' ) );
			} else {
				$this->make_output( false, __( 'Sorry sliders was not imported.', 'traveladvisor' ) );
			}
		}
	}
	 ...
	 * invoke traveladvisor_import_wp_data action
	 */
	function import_wp_data() {
		...
		do_action( 'traveladvisor_import_wp_data', $this );
	...
	/**
	 * Process attachments, zip into uploads directory
	 *
	 * @return Boolean Whether zip was successfully extracted or not.
	 */
	function process_attachments() {
		// Download attachments zip and extract it to local directory.
		$first_str_filename = '';
...
//			$zip->close();
	...
	/**
	 * Import Widgets
	 * invoke traveladvisor_import_widgets
	 */
	function import_widgets() {
		do_action( 'traveladvisor_import_widgets', $this );
	}

	/**
	 * Import Theme Options
	 * invoke traveladvisor_import_theme_options
	 */
	function import_theme_options() {
		do_action( 'traveladvisor_import_theme_options', $this );
	}

	/**
	 * Import Users
	 * invoke traveladvisor_import_users
	 */
	function import_users() {
		do_action( 'traveladvisor_import_users', $this );
	}

	/**
	 * Import Menus and Locations
	 * invoke traveladvisor_import_menus_and_locations
	 */
	function import_menus_and_locations() {
		do_action( 'traveladvisor_import_menus_and_locations', $this );
	}

	/**
	 * Import Plugin Options
	 * invoke traveladvisor_import_plugin_options hook
	 */
	function import_plugin_options() {
		do_action( 'traveladvisor_import_plugin_options', $this );
	}

	/**
	 * Import Sliders
	 * invoke traveladvisor_import_rev_sliders
	 */
	function import_sliders() {
		do_action( 'traveladvisor_import_rev_sliders', $this );
	}

	/**
	 * Set up Pages, set home page for the WP site
	 * invoke traveladvisor_import_setup_pages
	 */
	function set_up_pages() {
		do_action( 'traveladvisor_import_setup_pages', $this );
	}
