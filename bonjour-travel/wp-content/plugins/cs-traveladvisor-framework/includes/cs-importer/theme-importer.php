...
add_action( 'admin_menu', 'traveladvisor_theme' );
if ( ! function_exists( 'traveladvisor_theme' ) ) {
	/**
	 * Add Importer item to WP Admin Dashboard sidebar menu
	 */
	function traveladvisor_theme() {
		add_theme_page( 'Import Demo Data', __( 'Import Demo Data', 'traveladvisor_demo_importer' ), 'read', 'traveladvisor_demo_importer', 'traveladvisor_demo_importer' );
	}
}
...
if ( ! function_exists( 'traveladvisor_demo_importer' ) ) {
	/**
	 * Output GUI for Demo Importer
	 */
	function traveladvisor_demo_importer() {
		...
		if ( function_exists( 'get_server_requirements' ) ) {
			...
			wp_die( __( 'Server reuirements not available.', THEME_TEXT_DOMAIN ) );
		}
					...
					<div class="importer-steps-containers-wrapper importer-text row">
						<div class="step-1-container">
							<div class="col-lg-6 col-md-6 col-sm-12">
								...
								<div class="row">
									<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
										<a href="http://chimpgroup.com/wp-demo/documentation/documentation/traveladvisor-theme-guide/" class="importer-btn">
							...
							<div class="col-lg-6 col-md-6 col-sm-12">
								...
								<!--
								<h3><?php _e( 'Reason for giving away', THEME_TEXT_DOMAIN ); ?></h3>
								<p>
									...
									for the product.', THEME_TEXT_DOMAIN ); ?>
									
								</p>

								<h3><?php _e( 'How you can get free PSD', THEME_TEXT_DOMAIN ); ?></h3>
								<p>
									<?php _e( 'Give Review on item which you bought on ThemeForest by going to ThemeForest Downloads section.', THEME_TEXT_DOMAIN ); ?>
								</p>
								-->
								...
								<?php
									...
									if ( $envato_purchase_code_verification ) {
										if (
											...
											&& $envato_purchase_code_verification['last_verification_time'] + 30 * 24 * 60 * 60 > time()
										 ) {
											$verify_code = false;
										}
									}
						...
						<div class="step-3-container hidden">
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
								...
								<div class="sub-step-2 col-lg-5 col-md-5 col-sm-12 col-xs-12 hidden">
									...
									<?php
										$is_traveladviser_installed = isset( $plugin_requirements['traveladviser_var']['is_installed'] ) ? $plugin_requirements['traveladviser_var']['is_installed'] : false;
									...
									<ul class="job-select-demo">
										...
										<li class="users-chk-wrapper hidden <?php echo $is_traveladviser_installed ? '' : 'error'; ?>">
											<div class="radiobox">
												<input type="checkbox" id="chk-users" value="users" name="chk-users" class="checkbox-data-import-type" <?php echo $is_traveladviser_installed ? '' : 'disabled="disabled"'; ?>>
												<label for="chk-users" class="checkbox-data-import-type">
													<?php _e( 'Users', THEME_TEXT_DOMAIN ); ?>
													<?php echo $is_traveladviser_installed ? '' : '<br>Please install Job Hunt Plugin.'; ?>
												</label>
											</div>
										</li>
						...
						if( current_step == 1 ) {
							...
							if ( is_purchase_code_verified || is_purchase_code_already_verified ) {
								...
								$.ajax( {
									...
									"data": {
										"action": "traveladvisor_import_demo_data_callback",
...
add_action( 'wp_ajax_traveladvisor_import_demo_data_callback', 'traveladvisor_import_demo_data_callback' );

/**
 * Handle AJAX improt demo data calls
 */
function traveladvisor_import_demo_data_callback() {
	$remote_api_url = REMOTE_API_URL;
	$action_type = isset( $_POST['action_type'] ) ? $_POST['action_type'] : '';
	if ( 'import_data' == $action_type ) {
		if ( ! class_exists( 'CS_Data_Importer' ) ) {
			require_once wp_traveladvisor_framework::plugin_path() . '/includes/cs-importer/cs-data-importer.php';
		}
		...
		$traveladvisor_importer = new CS_Data_Importer();
		$traveladvisor_importer->demo_data_name = $selected_demo;
		...
		if ( 'content' == $import_type ) {
			...
			$traveladvisor_importer->homepage_slug				= $traveladvisor_importer->demo_data_name . '-home';
			$traveladvisor_importer->wp_data_path				= isset( $urls['content'] ) ? $urls['content'] : '';
			$traveladvisor_importer->menus_data_path			= isset( $urls['menus'] ) ? $urls['menus'] : '';
			$traveladvisor_importer->attachments_path			= isset( $urls['attachments'] ) ? $urls['attachments'] : '';
			$traveladvisor_importer->attachments_replace_url	= ATTACHMENTS_REPLACE_URL;
			$traveladvisor_importer->is_attachments_zip			= true;
			$traveladvisor_importer->is_content					= true;
		} elseif ( 'widgets' == $import_type ) {
			$traveladvisor_importer->widget_data_path = isset( $urls['widgets'] ) ? $urls['widgets'] : '';
			$traveladvisor_importer->is_widgets = true;
		} elseif ( 'options' == $import_type ) {
			$traveladvisor_importer->theme_options_data_path = isset( $urls['options'] ) ? $urls['options'] : '';
			$traveladvisor_importer->is_theme_options = true;
		} elseif ( 'users' == $import_type ) {
			$traveladvisor_importer->users_data_path = isset( $urls['users'] ) ? $urls['users'] : '';
			$traveladvisor_importer->is_users = true;
		} elseif ( 'menus' == $import_type ) {
			$traveladvisor_importer->menus_data_path = isset( $urls['menus'] ) ? $urls['menus'] : '';
			$traveladvisor_importer->is_menus = true;
		} elseif ( 'plugins' == $import_type ) {
			$traveladvisor_importer->plugins_data_path = isset( $urls['plugins'] ) ? $urls['plugins'] : '';
			$traveladvisor_importer->is_plugins = true;
		} elseif ( 'rev_slider' == $import_type ) {
			$traveladvisor_importer->sliders_data_path = isset( $urls['sliders'] ) ? $urls['sliders'] : '';
			$traveladvisor_importer->is_sliders = true;
			$traveladvisor_importer->sliders_options = isset( $_POST['other_data'] ) ? $_POST['other_data'] : array( false, false, false );
		}

		$traveladvisor_importer->import();
		...
		if ( $envato_purchase_code_verification ) {
			...
			if (
					$item_purchase_code == $envato_purchase_code_verification['item_purchase_code']
					&& $envato_purchase_code_verification['last_verification_time'] + 30 * 24 * 60 * 60 > time()
			 ) {
				$success = true;
			} else {
		...
		if ( ! empty( $envato_purchase_code_verification ) ) {
			...
			if ( isset( $urls[ $demo_data_name ] ) ) {
				...
				$output['conflicted_pages'] = traveladvisor_get_importer_conflicted_pages( $demo_data_name );
			}
		}
...
if ( ! function_exists( 'traveladvisor_get_importer_conflicted_pages' ) ) {
	/**
	 ...
	 */
	function traveladvisor_get_importer_conflicted_pages( $demo_data_name ) {
		$output = '';
		$confliced_pages_file = wp_traveladvisor_framework::plugin_path() . '/includes/cs-importer/conflicts.php';
